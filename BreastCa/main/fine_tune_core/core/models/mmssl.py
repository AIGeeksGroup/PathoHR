...
import sys
sys.path.append(r'/home/data2/zhangaho/1/zhangh/BreastCa/ViTAR')
from .main import AdaptiveTokenMerger
...
class MMSSL(nn.Module):
    def __init__(self, config, n_tokens_rna):
        ...
        print(self.n_tokens_wsi,self.patch_embedding_dim,self.n_tokens_rna)

        ########## WSI embedder #############
        if self.config["wsi_encoder"] == "abmil":
            assert self.config["n_heads"] == 1, "ABMIL must have only 1 head"
            adapt_params = {
                'dim': self.patch_embedding_dim,
                'heads': self.config['adapt_heads']
            }
...
class ABMILEmbedder(nn.Module):
    ...
    def __init__(
        ...
        if adapt_params is not None:
            self.head = adapt_params['heads']
            self.adapt = AdaptiveTokenMerger(**adapt_params)
    ...
    def forward(
        ...
        """
        # print(bags.shape) 
        # bags = AdaptiveTokenMerger(bags) # ()
        print(bags.shape) # (128,1024,1024) last (72,1024,1024)
